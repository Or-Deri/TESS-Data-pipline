...
docs/cleaning/dehazer/       # These docs (workflow/, PRD, thesis)
tests/                       # Test suite
  cleaning/                  # Dehazer / CleaningStage tests
  data/cleaning/groundtruth/ # raw/ + expected/ captured fixtures
...
Fixtures: [`tests/data/cleaning/groundtruth/`](../../../tests/data/cleaning/groundtruth/).
...
| Test | Scope |
|---|---|
| `cleaning/test_pipeline.py` | `dehaze` determinism + the `DehazePipeline` facade matches `dehaze` (synthetic FITS, `1e-9`) |
| `cleaning/test_groundtruth.py` | Full-res run on `tests/data/cleaning/groundtruth/raw` matches captured `expected/` (marked `slow`) |
| `cleaning/test_strategy.py` | Protocol / Strategy / Registry / `DehazingContext` layer |
| `cleaning/test_workflow_engine.py` | `Stage` / `Chain` engine (composition, ordering, guards) |
| `image_subtraction/` | ImageSubtractionStage + slow OIS residual groundtruth |
...
- Guided-filter backend: OpenCV `ximgproc` (float32) vs the NumPy/SciPy
  ...
  in [`tests/data/cleaning/groundtruth/`](../../../tests/data/cleaning/groundtruth/), use plain `opencv-python` (no
