...
|---|---|
| [test_pipeline.py](../../../../tests/cleaning/test_pipeline.py) | `dehaze` is deterministic for a fixed config, and the `DehazePipeline` facade reproduces `dehaze` output on synthetic FITS to `rtol=atol=1e-9`. |
| [test_groundtruth.py](../../../../tests/cleaning/test_groundtruth.py) | Full-resolution end-to-end run on `tests/data/cleaning/groundtruth/raw` matches the captured `tests/data/cleaning/groundtruth/expected` FITS. Marked `slow`; auto-skipped if the fixtures are missing. |
| [test_strategy.py](../../../../tests/cleaning/test_strategy.py) | The Strategy layer (`available_strategies() == ["default"]`, factory, registry guards, `DehazingContext`) and that the strategy wrapper is a no-op over `dehaze`. |
| [test_workflow_engine.py](../../../../tests/cleaning/test_workflow_engine.py) | The `Stage`/`Chain` engine: composition via `>>`, ordering, `requires` guards, `WorkflowContext.prefix`. |
...
- Automated suite: [tests/cleaning/](../../../../tests/cleaning) (`conftest.py`, `helpers.py`, `test_*.py`)
- Ground-truth fixtures: [tests/data/cleaning/groundtruth/](../../../../tests/data/cleaning/groundtruth/)
