...
**Status:** Partial implementation — **Cleaning** and **Image Subtraction** owned; other stages are *shields* (stubs).
...
```
Ingestion → Validation → Cleaning → Image Subtraction → Detrending → Normalization → Feature Extraction → Classification
```
...
|---|---|
| Cleaning | **Owned / implemented** — `Cleaner` + `CleaningStage` + dehazer |
| Image Subtraction | **Owned / implemented** — `Subtractor` + `ImageSubtractionStage` + OIS engine (`paloma.image_subtraction`) |
...
> `paloma.cleaning.dehazer` — see [`docs/cleaning/dehazer/README.md`](cleaning/dehazer/README.md)).
>
