...
**Status:** Design goals (still apply) — **Cleaning** and **Image Subtraction** are implemented under these principles; other stages remain shields.
...
This repository currently **owns cleaning and image subtraction** (`Cleaner` /
`CleaningStage` / dehazer; `Subtractor` / `ImageSubtractionStage` / OIS). Other
pipeline stages exist only as *shields* (stubs). The goals below still describe
the full system; these two areas are the first concrete realizations of
Extensibility, Configuration Flexibility, Testability, and Reusability via
config-selected registries.
...
**How the architecture supports it:** Cleaning is covered by `tests/` (synthetic FITS + optional full-resolution ground truth under `tests/data/cleaning/groundtruth/`). Each `PipelineStage` remains a standalone `run` method, so future stages can be unit-tested the same way.
